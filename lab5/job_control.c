...
job* initialize_job(char* cMd){
	job* newJob =malloc(sizeof(job));
        newJob->cmd=malloc(2048);
        strcpy(newJob->cmd,cMd);
        newJob->idx=0;
        newJob->pgid=0;//newJob->pgid=getpgid();
        newJob->status=0;
        newJob->tmodes=malloc(sizeof(struct termios));//tcgetattr(epoll(),newJob->termios);//struct cmdline {int s_in;int s_out;cmdline_parse_ctx_t *ctx;struct rdline rdl;char prompt[RDLINE_PROMPT_SIZE];struct termios oldterm;
        newJob->next='\0';
        return newJob;
}
...
void update_job_list(job **job_list, int remove_done_jobs)
{
    ...
    while(tmp!='\0')
    {
        job* jobToRemove=tmp;
        tmp=tmp->next;
        if(jobToRemove->status==1)
        {
            printf("test\n");
             printf("%d\n",jobToRemove->pgid);
            if(waitpid(jobToRemove->pgid,&status,WNOHANG)>0)
            {
               
                jobToRemove->status=-1;
                if(remove_done_jobs==TRUE)
                {
                    printf("[%d]\t %s \t\t %s", jobToRemove->idx, status_to_str(jobToRemove->status),jobToRemove -> cmd);
                    if (jobToRemove -> cmd[strlen(jobToRemove -> cmd)-1] != '\n')
			printf("\n");
                    remove_job(job_list,jobToRemove);
